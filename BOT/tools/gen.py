...
@Client.on_message(filters.command(["gen", ".gen", "$gen", "mod", ".mod", "$mod"]))
async def gen_command(client, message: Message):
    args = message.text.split()
    if len(args) < 2:
        return await message.reply(
            "<pre>Card Generator ❌</pre>\n"
            "<b>Usage:</b> <code>/gen {bin/extrap} {amount}</code>\n"
            "<b>Example:</b> <code>/gen 414720xxxxxxxxxx|xx|xx 10</code>",
            quote=True
        )

    extrap = args[1]
    amount = int(args[2]) if len(args) > 2 and args[2].isdigit() else 10
    amount = min(amount, 50)  # Limit max to 50
