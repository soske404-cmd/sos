...
def load_proxy_csv():
    ...
    try:
        ...
        print(f"[Proxy CSV Load Error] {e}")
    
    return proxies

# Load CSV proxies at startup
CSV_PROXIES = load_proxy_csv()

def get_random_csv_proxy() -> str | None:
    """Get a random proxy from CSV file"""
    if not CSV_PROXIES:
        return None
    return random.choice(CSV_PROXIES)

def get_system_proxy(user_id: int = None) -> str | None:
    """Get proxy - first checks user's proxy, then falls back to CSV proxy"""
    if user_id:
        user_proxy = get_proxy(user_id)
        if user_proxy:
            return user_proxy
    return get_random_csv_proxy()
...
@Client.on_message(filters.command("pxstatus") & filters.private)
async def proxy_status(client, message):
    ...
    reply_text = f"""<pre>Proxy Status 🌐</pre>
━ ━ ━ ━ ━━━ ━ ━ ━ ━
<b>⟐ CSV Proxy Pool:</b> <code>{len(CSV_PROXIES)} proxies</code>
<b>⟐ Your Proxy:</b> <code>{'Set ✅' if user_proxy else 'Not Set ❌'}</code>
━ ━ ━ ━ ━━━ ━ ━ ━ ━
<b>Commands:</b>
• /setpx - Set your proxy
• /getpx - View your proxy
• /delpx - Delete your proxy
"""
